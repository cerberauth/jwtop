...
For large wordlists, `jwtop` can optionally fall back to the external `john` or `hashcat` tools if its built-in dictionary attack doesn't find the secret (see [crack &raquo; External cracking tools](../reference/cli/crack/#external-cracking-tools)):
