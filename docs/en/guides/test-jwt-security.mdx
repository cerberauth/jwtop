...
See the [`crack` reference](../reference/cli/crack/#placing-the-jwt-elsewhere) for all `--token-in` options.
...
# Command injection variant (targets servers that shell out using kid)
jwtop exploit kidinjection --mode command $TOKEN
jwtop exploit kidinjection --mode command --all $TOKEN  # every shell payload variant

# LDAP injection variant (targets servers that use kid in an LDAP lookup)
jwtop exploit kidinjection --mode ldap $TOKEN
jwtop exploit kidinjection --mode ldap --all $TOKEN  # every LDAP payload variant
```

**JWK header injection (self-signed key, CVE-2018-0114):**
...
<LinkCard title="GitHub Actions guide" description="Full reference for using jwtop in CI/CD." href="../github-actions/" />

## See also

- [`crack` reference](../reference/cli/crack/)
- [`exploit` reference](../reference/cli/exploit/)
