...
| `fuzz` | Claim-mutation fuzzing — opt-in via `--fuzz`, skipped otherwise |

### Claim-mutation fuzzing (`--fuzz`)

`--fuzz` complements the fixed techniques above with a fuzzing pass: every claim is mutated in turn (type confusion, an oversized string, special characters, an explicit null) and each mutated response is compared against a reference response captured from the original, unmutated token — not the invalid-token baseline used by the other checks. A mutation is flagged when the response diverges:

- a `5xx` status code
- the body matches a known stack-trace/unhandled-exception signature
- the body length differs from the reference by more than 50%
...
See [JWT Claim Fuzzing](/vulnerabilities/jwt-claim-fuzzing/) for details.
