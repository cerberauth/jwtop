...
If you want to test if your server is vulnerable to the "JWT Alg None" vulnerability, you can use the `crack` command:

```bash
jwtop crack [token] --url [url]
```

To only generate a token with the "none" algorithm, use the `exploit algnone` command:
