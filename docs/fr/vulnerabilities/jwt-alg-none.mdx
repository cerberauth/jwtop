...
Si vous voulez tester si votre serveur est vulnérable à la vulnérabilité « JWT Alg None », vous pouvez utiliser la commande `crack` :

```bash
jwtop crack [token] --url [url]
```

Pour simplement générer un token avec l'algorithme « none », utilisez la commande `exploit algnone` :
...
Corriger la vulnérabilité JWT « alg none » consiste à s'assurer que la bibliothèque ou l'implémentation JWT utilisée n'est pas vulnérable à ce problème et est correctement configurée.
