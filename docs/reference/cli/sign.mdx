...
- When `<token>` is omitted, it is read from stdin — e.g. `jwtop find --file page.html | jwtop sign --alg HS256 --secret newsecret`.
...
- See [Strip or remove a token signature](../../guides/strip-signature) for context on when this is useful.
